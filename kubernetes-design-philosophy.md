# Kubernetes 设计哲学：取势 · 明道 · 优术

> Kubernetes 之名源于希腊语，意为「舵手 / 飞行员」。Google 于 2014 年开源该项目，将十余年大规模生产负载经验与社区最佳实践，凝练为一套可对外使用的容器编排平台。[1]
>
> 本库以「取势 · 明道 · 优术」为认知骨架。读 Kubernetes，亦当循此三境：
>
> | 境界 | 所问 | 在本文中的落点 |
> |------|------|----------------|
> | **取势** | 为何此时出现？上一代之势何以穷尽？ | 云可编程、容器化、复杂度下沉、Borg 经验外溢 |
> | **明道** | 可久可大的原则是什么？ | 一份真相、声明式收敛、控制面/数据面分离、平台之平台 |
> | **优术** | 原则如何落成可试点的工艺？ | 分层高可用、L4 入口、选主、探针、CRD / Operator |
>
> 不明势，则把营销当定律；不通道，则困于 YAML 与名词；不精术，则道理空悬。三者相生，缺一则偏。
...
>
> 用一句道家语言收束：**顺故障之势，立收敛之道，成分层之术。**

全文可与本库 [《服务架构演进》](./service-architecture-evolution.md)（复杂度如何转移）、[《分布式系统理论》](./distributed-systems-theory.md)（CAP / Raft）对照阅读。关键史实与论断尽量对齐一手文献，文末附参考文献。
...
1. [一条主线：势、道、术如何层层展开](#1-一条主线势道术如何层层展开)
2. [时代之势：为何需要「舵手」](#2-时代之势为何需要舵手)
3. [史脉之势：Borg → Omega → Kubernetes](#3-史脉之势borg--omega--kubernetes)
4. [定位之势：是什么、不是什么](#4-定位之势是什么不是什么)

**明道 · 立本于不败**

5. [云基础设施的三个前提](#5-云基础设施的三个前提)
6. [核心理念：Platform for Platform](#6-核心理念platform-for-platform)
7. [道之一：一份真相与松耦合协调](#7-道之一一份真相与松耦合协调)
8. [道之二：持续收敛与静态稳定](#8-道之二持续收敛与静态稳定)
9. [道之器：控制平面、API 与控制器](#9-道之器控制平面api-与控制器)
10. [设计原则精要](#10-设计原则精要)

**优术 · 成事于精微**

11. [分层高可用：把道落成故障域策略](#11-分层高可用把道落成故障域策略)
12. [入口之术：API Server 负载均衡](#12-入口之术api-server-负载均衡)
13. [扩展之术：CRD 与 Operator](#13-扩展之术crd-与-operator)
14. [能力边界与检查清单](#14-能力边界与检查清单)

**收束**

15. [总结：势道术合一](#15-总结势道术合一)
16. [参考文献](#16-参考文献)

```mermaid
flowchart TB
  Shi["取势<br/>时代 · 史脉 · 定位"]
  Dao["明道<br/>真相 · 收敛 · 静稳 · 平台"]
  Shu["优术<br/>分层 HA · LB · 选主 · Operator"]

  Shi -->|"知所从来"| Dao
  Dao -->|"准则可守"| Shu
  Shu -.->|"术反哺道<br/>道须合势"| Shi
```
...
|:----:|:----:|----------|----------|
| **①** | 取势 | 上一代运维 / 编排之势何以穷尽？ | §2 |
| **②** | 取势 | 经验从 Borg 到开源如何外溢？ | §3 |
| **③** | 取势 | 它在浪潮中站在哪一格？ | §4 |
| **④** | 明道 | 云土壤给出哪些不可违背的前提？ | §5–§6 |
| **⑤** | 明道 | **分布式精髓与可久原则是什么？** | **§7–§10** |
| **⑥** | 优术 | **高可用与入口如何落地？** | **§11–§12** |
| **⑦** | 优术 | 同一套道如何扩展到任意领域？ | §13–§14 |

> **读法**：先问「为何此时」，再立「何以可久」，最后落「如何试点」。次序不可颠倒——有术无道是投机，有道无势是空悬，有势无术是空过。
...
上一代「命令式编排 / 手工剧本」之势已穷：规模一大，故障组合爆炸，逐步脚本既不经济，也不诚实。行业需要的不是更长的 Runbook，而是**把故障当成稳态输入、用收敛代替剧本**的控制平面。

> **取势之判**：Kubernetes 应时而生——不是发明了容器，而是为「可编程云 + 不可变制品 + 微服务后的运维洪峰」提供了舵手。

---

## 3. 史脉之势：Borg → Omega → Kubernetes
...
> **取势之判**：开源的不是 Borg 的皮肉，而是「规模下故障是常态」这一假设——把内部道，化为外部可学的术。
...
> **要点**：Kubernetes 管「如何声明与收敛」；生态管「具体实现插件」。用**持续收敛**替代中心化剧本——此即势之所趋的最短表述。

---

## 5. 云基础设施的三个前提

道不离势。Kubernetes 能立，建立在云把基础设施「产品化」之后的三个前提之上：
...
Kubernetes 的道之定位是 **Platform for Platform**——用来构建分布式系统的分布式系统。首要用户是分布式应用开发者。

「平台之平台」意味着：不把所有领域知识写死在核心里，而把**声明与收敛的能力**开放出去。上层经 CRD / Operator 扩展，而不必每次重造控制平面。官方要求与此一致：可扩展、可自动化；声明式是自愈的关键。[8]

> **明道之判**：复杂度不会消失，只会转移——Kubernetes 把「如何协调分布式」收成平台能力，把「协调什么」留给领域。
...
优术，是道在事上的落实。《庄子》云庖丁「依乎天理……因其固然」——高可用亦然：按故障域分层，而非口号式多副本。

| 层 | 手段 | 所成之道 |
|:--:|------|----------|
| **①** | etcd / Raft 共识 | 纪律一：一份真相 |
| **②** | API Server + L4 LB | 入口可达 |
| **③** | Lease 选主 | 单一活跃调谐者 |
| **④⑤** | kubelet + 负载自愈 | 业务连续性 |
| **—** | 控制面 / 数据面分离 | 纪律二：静态稳定 |
...
## 13. 扩展之术：CRD 与 Operator

若止于内置资源，Kubernetes 只是调度器。Platform for Platform 靠**同一套收敛之术可被领域复用**：
...
> **术以载道**：先统一「如何描述、如何共识、如何在故障下收敛」，再让各领域填写「收敛什么」——愿景中的「通用软件控制平面」，不过是道的外推。
...
| 境界 | 命题 | 要点 |
|------|------|------|
| **取势** | 时代与史脉 | 云可编程 × 容器不可变 × 复杂度下沉；Borg/Omega 经验外溢，非 Borg 开源版[2] |
| **明道** | 可久原则 | 一份真相 · API 松耦合 · level-based 收敛 · 静态稳定 · Platform for Platform[12][19] |
| **优术** | 落地工艺 | 共识 → L4 入口 → 选主 → 节点闭环 → 负载自愈；CRD/Operator 外推；边界清晰[13][25] |

| 偏废 | 后果 |
|------|------|
| 有术无道 | 堆 HAProxy / 探针，却不懂为何停写、为何静稳——技巧失据 |
| 有道无势 | 空谈声明式，却无视云与容器尚未成熟时不可强行——理想空悬 |
| 有势无术 | 看清浪潮，却无分层 HA 与入口工艺——机遇空过 |

> **收束**  
> 取势：顺「故障是常态」的时代流向。  
> 明道：立「一份真相、持续收敛、静态稳定」于不败。  
> 优术：成「分层高可用与可扩展控制平面」于精微。  
>
> 势为时，道为理，术为器——**先取势以定方向，再明道以立准则，后优术以致事功。**  
> 舵手之意，不在无风浪，而在有道可依、有术可操，于变局中仍能指向可用。
