...
这些规划方法在市场上有不同的名称，包括战略信息系统规划、战略数据规划、信息架构、企业架构。**本节时间窗是商业计算与规划方法的前史（约 1959—1980）**；下表前三行落在这一窗内。后三行是同一套「现状—目标—路线图」后来被冠上的名称，Kotusev 把它们收进同一谱系，公开可核验的发布年见 3.3–3.6（例如 TOGAF Version 1 为 1995 年，并非 2010 年代才出现）。
...
> **本节要点：** 五个词所指不同；建设宜先业务，再应用与数据，再技术与代码，不宜长期倒置。
...
企业架构实践不应与下列事情相混淆。这是 Kotusev 原书总目；后文展开与此不完全同序：6.4 对应纯技术规划，6.6 对应一次性规划，6.7–6.8 对应建模 / 工程 / 系统思考，6.10 对应框架实施。6.5、6.9 的「万能方法」与「突破性方案」是同章相邻判断，未写入上列。
...
- 2023 年国资委通报：89 家央企明确数字化转型发展规划；90 多家组建「一把手」负责机制；要求业务 / 数据 / 应用 / 技术架构融合一致。[^sasac-digital2023] 相关规划口径亦可对照《国有企业数字化转型工作指南》等文件中对业务 / 应用 / 技术 / 数据等架构安排的要求。[^sasac-guide]
...
Kotusev 提醒我们：实践企业架构主要是指沟通和行动，而非思考；它不是一次性规划、不是填满某个框架单元格，也不是推销员兜售的突破性方案。[^kotusev-pea]  
