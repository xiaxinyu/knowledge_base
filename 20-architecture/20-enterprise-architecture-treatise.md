...
企业架构（Enterprise Architecture，EA）把战略翻译为可执行的经营与 IT 安排，并持续约束该建什么、不该建什么。针对「人人都在谈架构、所指却不同」所导致的系统堆砌、口径分裂与维护成本上升，全文从定义与价值机制、TOGAF 等源流、业务 / 应用 / 数据 / 技术四大域及其与代码架构的关系，写到常见误区、中外案例与分规模落地路径。EA 不是开发框架，也不是几张架构图。

**关键词：** 企业架构；TOGAF；四大架构域；战略落地
