...
专论分布式数据在跨节点之后如何取舍。单机用 ACID 把正确性收进事务；分区出现后，CAP 迫使在强一致与持续可用之间选择。取舍之后有两条工程路径：AP 走 BASE 与最终一致，CP 走复制状态机与 Paxos / Raft / ZAB。中篇对照三种共识，并交代 FLP 为何必须借助超时；下篇谈同一系统中 ACID 与 BASE 并存及选型。定义尽量对齐一手文献，避免把 CAP 当成三选一的口号。

**关键词：** ACID；CAP；BASE；Paxos；Raft；ZAB
