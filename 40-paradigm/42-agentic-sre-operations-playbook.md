...
监控加「AI」标签并不自动缩短故障恢复时间。可核对的价值，是压缩平均恢复时间（MTTR）中可并行、可检索、可证据化的时间段，而不是承诺无人值守自愈。经典 AIOps / 事件智能主要缩短「发现」；大模型加检索与智能体主要缩短「理解、取证与协同」。生产上不要用一个大模型包办全链路：窄任务交给小模型（SLM），跨源解释交给大模型（LLM），工具编排与权限门闸交给 Agent，写操作默认人在环上。文中数字均为公开观察，须用本组织基线复核。

**关键词：** MTTR；SLM；LLM；Agent；AIOps；人在环上
...
| **DORA：AI 是放大器** | 2025 报告：AI 放大高绩效组织的优势，也放大脆弱组织的缺陷；高质量内部平台显著影响 AI 收益；AI 与吞吐正相关，与交付稳定性仍可能负相关[^dora-2025]；研发侧个人吞吐亦见 Copilot 受控实验[^peng-2023] | 可观测性乱、变更无规范、服务目录缺失时，Agent 会更快放大混乱 |
| **错误预算与 SLO** | SRE 语言是 SLO / 错误预算，不是模型准确率 | 自动化误操作应消耗错误预算；预算耗尽则收回自治权；涉生产数据与对外生成式服务时对齐境内生成式 AI 合规底线[^cn-gai] |
