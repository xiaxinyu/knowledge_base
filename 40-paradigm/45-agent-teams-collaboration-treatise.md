...
这里有一个人脑和大模型的根本区别：人今天学会了 Java，晚上突触就长出来了，明天照样会写——学习这件事物理性地改写了大脑本身。大模型完全相反：它跑完一件事、「解决」了一个问题，参数权重一个字节都不会变，下次启动还是那个出厂模型；如果我们不把这次的经验、结论、教训持久化到外部存储、下次通过 context 再喂回去，等于什么都没发生过。

所以 Agent 不会「自动变强」——它只在你为它建了外部记忆的那部分变强。这也是为什么 Agent 工程绕不开 RAG、记忆压缩、Skill 注入、知识库、轨迹仓库这一整套东西——它们本质都是在替大模型做那件它自己做不到的事：把经验存起来、需要时精准塞回有限的 context window。

学术界的持续学习方向（例如 2026 年前后的 LLM 持续学习综述工作）在持续预训练、持续微调（LoRA 变体）、持续对齐上都有进展，但核心难题「灾难性遗忘」——学了新东西旧的就忘——远未根本解决。[^continual-2026] 在这个问题被彻底解决之前，「上下文即记忆」仍是生产上最可依赖的工程路径；持续学习更多是研究与受控微调的补充，而不是替代外部记忆系统。

即使有一天持续学习成熟、模型能像人一样在使用中自我进化，「行动」和「观察」仍是工程团队不可替代的战场——模型再聪明，没有好工具就做不了事；多 Agent 场景下，没有共享工件与通信预算，再强的个体也会在协作缝上失分。

本库 [40](./40-unix-agent-stateless-philosophy.md) 从 Unix 到 Agent 论证过同一结构：无状态不是「不保存任何数据」，而是审慎选择状态的存放位置与生命周期；Agent 的「记忆」应落在外部可组合介质上，而不是幻想模型内部自动长出突触。ICLR 2026 前后围绕 *Memory for Agentic Systems* 的工作坊议程，也把「记忆」明确标为 Agent 系统的一等议题。[^iclr-memory]
