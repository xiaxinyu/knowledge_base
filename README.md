...
- **文内一级标题**：应能看出主题与体裁，与文件名同向；避免过宽的讲座名。文件名是检索键，标题是阅读入口，二者不必逐字相同，但不得指到另一篇文章。  
- **摘要**：每篇正式文稿在目录前设「摘要」与「关键词」：一段话交代问题、主线与结论，不替代正文论证。
...
│       零售私有云平台服务质量案例：SERVQUAL 差距、TQM 根因与 PDCA 闭环
│
├── 40-paradigm/                           优术 · 智 · 新章
...
**50-strategy 为何三篇并列：** 本区回答「价值如何取胜」。50 看产业与竞争结构如何被 AI 穿透；51 看品牌意义如何跨文化传递；52 看 Token 如何成为智能经济的计量与计价单位。三者分别是局、译、币——同属优术之略，不是同一篇文章的上下篇。
...
3. **52** Token Economics Treatise — 再看智能经济如何计价
4. **11** Information Theory Chronicle — 回看信息的度量如何成为智能的基础

### 路径五：快速浏览（30 分钟版）

> 目标：不深入细节，只抓每篇核心判断。

每篇文档开头都有 `>` 引用的「直接答案」，目录前另有「摘要」——只读这些，约 30 分钟可扫完。若某篇引起兴趣，再沿目录深入。宽表格与 ASCII 图请用等宽字体、宽屏阅读。
...
- **明道而写**：关键判断尽量对照可核验来源；提案与史实、标准与个人见解，边界分明。同一篇内引用体例须统一：`10` / `20` / `30` 与 `50` 多用 `[n]` 编号表；`40` 与 `51` / `52` 多用 `[^name]` 脚注（含 URL）。暂不强制全库改成一种，以免一次替换打断已核验的出处。  
